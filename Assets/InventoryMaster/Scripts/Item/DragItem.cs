    ...
    public void OnEndDrag(PointerEventData data)
    {
        if (data.button == PointerEventData.InputButton.Left)
        {
            ...
            else
            {
                ...
                Vector3 tmp = GameObject.FindGameObjectWithTag("Player").transform.localPosition;
                Vector3 tmp2 = new Vector3(tmp.x + 0.4f, tmp.y + 1.0f, tmp.z); /*ATTENTIO j'ai modifier le script ici pour jeter lobject un peu plus haut que prevu !!*/ 
                dropItem.transform.localPosition = tmp2;
                inventory.OnUpdateItemList();
